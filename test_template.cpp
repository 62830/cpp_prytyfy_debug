...
//#define PRYTYFY(U,B,S,E) template<typename ...T> ostream& operator<<(ostream&O,U<T...> a){ O<<B; int c=0; for(auto i:a)O<<(c++?S:"")<<i; return O<<E; } template<typename ...T> void _print(const char *s,U<T...> a,bool nxt_line = false,int idt = 0,int dim = Dim<U<T...>>::val,typename enable_if< (Dim<U<T...>>::val>1) >::type* = 0){ bool b = strlen(s); if(b) cerr<<s<<" =\n";if(nxt_line) cerr<<"\n"; cerr<<B; int c=0; string sep = S; for(int i=0;i<dim-1;++i)sep += "\n"; for(int i=0;i<idt+1;++i)sep += " "; for(auto i:a){ cerr<<(c++?sep:""); _print("",i,0,idt+1,dim-1); } cerr<<E; cerr<<(b?"\n":""); return; }
/*template<typename ...T>
ostream& operator<<(ostream&O,U<T...> a){
	O<<B;
	int c=0;
	for(auto i:a)O<<(c++?S:"")<<i; return O<<E;
}
template<typename ...T>
void _print(U<T...> a,bool nxt_line = false,int idt = 0,int dim = Dim<U<T...>>::val,typename enable_if< (Dim<U<T...>>::val>1) >::type* = 0){
	if(nxt_line) 
		cerr<<"\n"; 
	cerr<<B; 
	int c=0; 
	...
	for(auto i:a){ 
		cerr<<(c++?sep:""); 
		_print(i,0,idt+1,dim-1); 
	}
	cerr<<E; 
	return; 
}
template<typename T>
void _print(T a,bool nxt_line = false,int idt = 0,int dim = Dim<T>::val,typename enable_if< (Dim<T>::val<=1) >::type* = 0){
	cerr<<a;
}
template<typename T>
void _print(T a,bool nxt_line = false,int idt = 0,int dim = Dim<T>::val){
	cerr<<"pointer"<<endl;
}
PRYTYFY(vector,"[",", ","]");
PRYTYFY(deque,"[",", ","]");
...
template<typename ...T> void _print_all(const char*s,T... a){
	cerr<<s<<" = ";
	int c=0;
	((cerr<<(c++?", ":""),_print(a,true)),...);
	cerr<<"\n";
}*/
/*PRYTYFY(vector,"[",", ","]"); 
PRYTYFY_PAIR(pair,"(",",",")");
